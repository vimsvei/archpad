...
**Причина:** Использование якоря (anchor) `*get_commit_sha` напрямую в массиве `before_script` не поддерживается в GitLab CI.

**Решение:** Развернул якорь напрямую в `before_script` шаблона `.build_docker_image`.
