/**
 * Server logger — unified format: timestamp, level, context.
 * Shared across landing, portal, and other apps for consistent log parsing in K8s.
 */
...
export type ServerLogger = {
  ...
  child(context: string): ServerLogger;
};

const createLogger = (defaultContext?: string): ServerLogger => ({
  log(message, context) {
...
/** Create a logger with a fixed context prefix (e.g. "landing", "portal"). */
