...
import { removeComponent } from "@/store/slices/solution-edit-slice"
...
export function ComponentsTable({
  ...
  const items = editState.components
  ...
  const handleDelete = React.useCallback(
    (item: RelatedItem) => {
      dispatch(removeComponent(item.id))
  ...
  return (
    <RelatedItemsMapTab<RelatedItem>
      ...
      editPath={(item) => `/portfolio/application/components/${item.id}`}
      onRefresh={handleRefresh}
      actions={{ onCreate, onAddExisting, onDelete: handleDelete }}
      selection={{ selectedItems, onToggleItem: handleToggleItem }}
      emptyState={{ componentName: solutionName, itemTypeKey: "components" }}
    />
  )
}