...
export function ListPage() {
  ...
  const [createItem, createState] = useCreateDataObjectMutation()
    ...
    () => [
      {
        ...
        header: "",
        enableHiding: false,
        cell: () => <ArchimateObjectIcon type="application-data-object" className="text-foreground opacity-80" />,
      },
      {
        accessorKey: "code",
        header: t("table.code"),
        cell: ({ row }) => (
          <Link
            href={`/application/data-objects/${row.original.id}`}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem asChild>
                    <Link href={`/application/data-objects/${item.id}`}>{t("action.edit")}</Link>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
